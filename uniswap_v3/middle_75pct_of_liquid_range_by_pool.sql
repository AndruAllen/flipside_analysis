...
current_positions_by_day AS (
  ...
  p.price_upper_1_0,
  p.blk_date,
  sum(case when p.block_id = mb.max_block_id then p.liquidity_adjusted else 0 end) as max_liquidity_adjusted,
  sum(case when p.block_id = mb_min.min_block_id then p.liquidity_adjusted else 0 end) as min_liquidity_adjusted
  ,avg(p.liquidity_adjusted) as avg_liquidity_adjusted
...
WHERE p.liquidity_adjusted > 0
  -- and (p.liquidity_adjusted > 0)
...
,  current_tvl as (
select tbl.*, row_number() over (partition by pairing_type order by tbl.tvl DESC) as rn
  from
(SELECT 
  pool_address, pool_name, --tick,
  token0_symbol, token1_symbol,
  case when token0_symbol in ('USDC','USDT','DAI','FEI','EURS') and token1_symbol in ('USDC','USDT','DAI','FEI','EURS') then 'Stable_Stable'
  		when token0_symbol in ('USDC','USDT','DAI','FEI','EURS') or token1_symbol in ('USDC','USDT','DAI','FEI','EURS') then 'Stable_Unstable'
  		else 'Unstable_Unstable' end as pairing_type
  ,date_trunc('day', block_timestamp) AS blk_date
...
,sum(zeroifnull(pool.avg_liquidity_adjusted)) as avg_liquidity_adjusted
...
-- select * from current_tvl where rn <= 5
, curr_ranges as (select a.blk_date, a.pool_name, price_bound_1_0, price_bound_running_total
from
(select a.blk_date, a.pool_name, price_bound_1_0
  -- , sum(case when price_bound_1_0 <= price_upper_1_0 and price_bound_1_0 >= price_lower_1_0 then avg_liquidity_adjusted else 0 end) 
  -- 			over(partition by blk_date, pool_name order by price_bound_1_0 ASC) as price_lower_running_total
  , sum(case when price_bound_1_0 <= price_upper_1_0 and price_bound_1_0 >= price_lower_1_0 then avg_liquidity_adjusted else 0 end) as price_bound_running_total
  -- , price_lower_running_total/total_liquidity
from tvl_by_day_lp_pool_band a
  left join
  (select distinct blk_date, pool_name, price_bound_1_0
  from (
  (select distinct blk_date, pool_name, price_lower_1_0 as price_bound_1_0 from tvl_by_day_lp_pool_band)
  union all
  (select distinct blk_date, pool_name, price_upper_1_0 as price_bound_1_0 from tvl_by_day_lp_pool_band))) b
  on a.blk_date = b.blk_date and a.pool_name = b.pool_name
where a.blk_date = CURRENT_DATE()
group by 1,2,3
order by price_bound_1_0 ASC) a)


select blk_date, pool_name
  , min(price_bound_1_0) as price_bound_1_0_min
  , max(price_bound_1_0) as price_bound_1_0_max
  , 1/price_bound_1_0_min as price_bound_0_1_min
  , 1/price_bound_1_0_max as price_bound_0_1_max
  from
  (select a.blk_date, a.pool_name, price_bound_1_0, price_bound_running_total
   , sum(price_bound_running_total) over(partition by a.blk_date, a.pool_name order by price_bound_1_0 ASC) as price_lower_running_total
   , sum(price_bound_running_total) over(partition by a.blk_date, a.pool_name order by price_bound_1_0 DESC) as price_higher_running_total
   , price_lower_running_total/tot_liquid as pct_lower
   , price_higher_running_total/tot_liquid as pct_higher
  
  from curr_ranges a
  left join (select blk_date, pool_name, sum(price_bound_running_total) as tot_liquid from curr_ranges group by 1,2) b
  on a.blk_date = b.blk_date and a.pool_name = b.pool_name
qualify pct_lower>= .1 and pct_higher>= .1 -- capture at least 75% of pool liquidity by looking at middle ~80%
order by price_bound_1_0 ASC)
group by 1,2
-- , current_top_lps as (
